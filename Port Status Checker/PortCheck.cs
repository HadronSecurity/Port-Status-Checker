...
namespace Port_Status_Checker
{
...

    {
        ...
        private async Task<bool> CheckTCPPortAsync(string ip, int port)
        {
            try
            {
                using (TcpClient tcpClient = new TcpClient())
                {
                    ...
                    {
                        return tcpClient.Connected; // Bağlantının gerçekten kurulduğunu kontrol et
                    }
                    else
                    {
        ...
        private async void PortCheckForm1_Load(object sender, EventArgs e)
        {
            ...
            // Firewall durumunu al
            string firewallStatus = GetFirewallStatus14();

            // Switch'i durumuna göre ayarla
            if (firewallStatus == "OFF")
            {
                guna2ToggleSwitch2.Checked = false; // OFF konumuna getir

            }
            else if (firewallStatus == "ON")
            {
                guna2ToggleSwitch2.Checked = true; // ON konumuna getir
        ...
        private void guna2ToggleSwitch2_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (enable)
            {
                ...
                label4.Text = "Windows Defender Firewall is now enabled.";
                label4.ForeColor = Color.Green;  // Yeşil renk
            ...
            else
            {
                ...
                // label6'ya durumu yazdır ve rengi değiştir
                label4.Text = "Windows Defender Firewall is now disabled.";
                label4.ForeColor = Color.Red;  // Kırmızı renk
            }
            SaveFirewallStatus();
        ...
        private void SaveFirewallStatus()
        {
            try
            {
                ...
                string logEntry = $"[{currentDate}] Firewall Status: {label4.Text}";
        ...
        private async void guna2CheckBox1_CheckedChanged(object sender, EventArgs e)
        {
            if (guna2CheckBox1.Checked)
            {
                try
                {
                    ...
                    {
                        MessageBox.Show("No internet connection!", "Connection Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                        return;
                    }
                ...
                {
                    // Hata oluşursa mesaj kutusu ile uyarı ver
        ...
        public static void LogUpnpData(RichTextBox richTextBox1, RichTextBox richTextBox2)
        {
            string folderPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "Port Check");
            string filePath = Path.Combine(folderPath, "PortHistory.txt");
            ...
            // RichTextBox1 ve RichTextBox2'deki metinleri al
            string logMessage = "RichTextBox1 İçeriği:\n" + richTextBox1.Text + "\n";
            logMessage += "RichTextBox2 İçeriği:\n" + richTextBox2.Text + "\n";
            logMessage += "Tarih: " + DateTime.Now + "\n";
            logMessage += new string('-', 50) + "\n"; // Loglar arasına ayırıcı çizgi ekler
            ...
            {
                MessageBox.Show($"Log kaydederken hata oluştu: {ex.Message}");
            }
        }
    }
}
